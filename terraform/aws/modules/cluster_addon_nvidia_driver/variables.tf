...
variable "gpu_worker_node_group_labels" {
  nullable    = false
  description = "The nvidia gpu plugin will use these node labels as node selectors when scheduling the pods to gpu nodes. At least one label is required."
  type = list(object({
    key   = string
    value = string
  }))
...
variable "gpu_worker_node_group_taints" {
  nullable    = false
  description = "It is assumed that gretel gpu nodes will be tainted so that regular workloads are not scheduled on them. Define these node taints so that the gpu driver can tolerate them when scheduling the drive pods."
  type = list(object({
    key    = string
    value  = string
    effect = string
