locals {
  ...
  gpu_model_worker_node_group_labels = var.gpu_model_worker_node_group_labels
  gpu_model_worker_node_group_taints = var.gpu_model_worker_node_group_taints
}
...
resource "helm_release" "nvidia_gpu_device_daemonset" {
  ...
  values = [yamlencode({
    affinity = {
      nodeAffinity = {
        requiredDuringSchedulingIgnoredDuringExecution = {
          nodeSelectorTerms = [
            {
              matchExpressions = [for label in local.gpu_model_worker_node_group_labels :
                {
                  key      = label.key
                  operator = "In"
                  values = [
    ...
    tolerations = [for taint in local.gpu_model_worker_node_group_taints :
      {
        key      = taint.key
        operator = "Equal"
        value    = taint.value
